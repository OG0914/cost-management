<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    ...
    <div class="stat-card stat-card-primary group">
      <div class="p-5 flex-1 relative">
        <div class="flex items-start justify-between mb-3">
          ...
          <div v-if="stats.growthRate" class="stat-trend stat-trend-up">
        ...
        <!-- 迷你趋势线 -->
        <div class="absolute bottom-3 right-4 opacity-30 group-hover:opacity-50 transition-opacity">
    ...
    <div class="stat-card group">
      ...
      <div class="stat-footer">
        <span class="truncate flex-1" :title="regulationNames">覆盖 {{ regulationNames }}</span>
      </div>
    </div>
    ...
    <div class="stat-card group">
      <div class="p-5 flex-1">
        ...
        <div class="mt-4">
          ...
          <h3 class="stat-label mt-1">有效原料 SKU</h3>
        </div>
      </div>
      <div class="stat-footer">
        <span>数据库记录</span>
    ...
    <div class="stat-card group">
      ...
      <div class="stat-footer">
        <span class="truncate">折叠 / 杯型 / 平面 / 半面罩</span>
      </div>
    </div>
  </div>
</template>
...
.stat-value {
  @apply text-3xl font-extrabold text-slate-800 tracking-tight;
...
.stat-footer {
  @apply bg-slate-50/80 px-5 py-2.5 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500;
}
.stat-trend {
